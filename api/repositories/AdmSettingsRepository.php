...
class AdmSettingsRepository implements IAdmSettingsRepository
{
    public function getUsersByLoggedUseId(int $idLoggedUser): array
    {
        ...
        $user->LastLoginAttempt = $loggedUser->LastLogin ? new \DateTime($loggedUser->LastLogin) : null;
        ...
        if ($user->UserRoleValue === UserRoleEnum::ADMIN) {
            $childUsers = \dibi::select('*')
                ->from('login')
                ->as('l')
                ->where('l.IdParent = %i', $idLoggedUser)
                ->fetchAll();
            ...
            foreach ($childUsers as $childUser) {
                ...
                $user->LastLoginAttempt = $childUser->LastLogin ? new \DateTime($childUser->LastLogin) : null;
